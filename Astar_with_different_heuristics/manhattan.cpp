...
int manhattan(char c , int a, int b, vector<vector<char>>& goal_state)
{
    for(int i=0;i<3;i++)
    {
        for(int j=0;j<3;j++)
...
int heuristic_func(vector<vector<char>>& state, vector<vector<char>> & goal_state)
{
    int ans=0;
    ...
    for(int i=0;i<3;i++)
    {
        for(int j=0;j<3;j++)
        {
            ans+=manhattan(state[i][j], i, j, goal_state);
        }
    }
    return ans;
}
...
{   
    ...
    pq.push({heuristic_func(start_state, goal_state)+0, {0, {start_state, ""}}});
       ...
       if(set_push(s, temp.second.second.first)) continue;
        ...
        for(int d=0;d<4;d++)
        {
            ...
            if(inRange(a, b, 3, 3))
            {
                ...
                pq.push({heuristic_func(temp.second.second.first, goal_state)+temp.second.first+1, {temp.second.first+1, {temp.second.second.first, dirc}}});
